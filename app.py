...
def clean_xml_text(value):
    """Remove NULL bytes and XML-illegal control characters. Safe for Word/XML."""
    if value is None:
        return ""
    value = str(value)
...
_MS_INSTR_PAT  = re.compile(
    r"Subject Details.*Markscheme|"
    r"Candidates are required to answer|"
...
_MS_BIOLOGY_INSTR_PAT = re.compile(
    r"Candidates are required|Subject Details|"
    r"Marking points are awarded|Examiners should be|"
    r"Section B.*Extended response|"
    r"quality of construction|"
    r"clarity of communication|"
    r"abbreviations.*award",
    re.I | re.DOTALL
)
...
def _extract_ms_text(ms_doc, start_pi: int, end_pi: int,
    ...
    Skips CircoBooks watermarks (w > 38% of page width).
    Returns list of dicts or None if actual diagrams/equations found.
    """
    import re as _re
    ...
    for pi in range(start_pi, min(end_pi + 1, len(ms_doc))):
        ...
        for blk in page.get_text("dict").get("blocks", []):
            if blk.get("type") == 1:
                ...
                iw  = bb[2] - bb[0]
                # Skip watermarks (> 38% of page width)
                if iw / max(pw, 1) > 0.38:
                    continue
                # Real content image in content area → use image rendering
                if bb[1] > ph * 0.10 and bb[3] < ph * 0.93:
                    return None
        ...
        for blk in page.get_text("dict").get("blocks", []):
            ...
            for ln in blk.get("lines", []):
                ...
                if y0 < ph * 0.10 or y0 > ph * 0.93:
                    continue
                if _re.match(r"^Section\s+[AB]$", line_txt, _re.I):
                    continue
                if line_txt in ("Question", "Answers", "Notes", "Total"):
                    continue
                if _re.search(
                    ...
                    continue
                if x0 > TOTAL_L and _re.match(r"^\d{1,4}$", line_txt) and y0 < ph * 0.15:
                    continue

                # Detect italic spans
                is_italic = any(bool(s.get("flags", 0) & 2) for s in spans)

                y_key = int(y0 / ROW_H) * ROW_H
                if y_key not in rows:
                    rows[y_key] = {"q": [], "ans": [], "notes": [], "total": []}

                tagged = ("i:" + line_txt) if is_italic else line_txt
                if x0 < ANS_L:
                    rows[y_key]["q"].append(line_txt)
                elif x0 < NOTES_L:
                    rows[y_key]["ans"].append(tagged)
                elif x0 < TOTAL_L:
                    rows[y_key]["notes"].append(line_txt)
                else:
                    rows[y_key]["total"].append(line_txt)
    ...
    for y_key in sorted(rows):
        r     = rows[y_key]
        q_t   = " ".join(r["q"]).strip()
        tot_t = " ".join(r["total"]).strip()
        not_t = " ".join(r["notes"]).strip()

        if q_t:
            if cur_ans or cur_notes:
                _flush(cur_part, cur_ans, cur_notes, cur_total)
            m = _re.search(r"(\d{1,2})\.\s*([a-z])|^([a-z])$", q_t)
            cur_part  = m.group(2) or m.group(3) or q_t if m else q_t
            cur_ans   = list(r["ans"])
            cur_notes = [not_t] if not_t else []
            cur_total = tot_t
        else:
            cur_ans.extend(r["ans"])
            if not_t:
                cur_notes.append(not_t)
            if tot_t and not cur_total:
                cur_total = tot_t
...
def _s_run(p, txt, bold=False, italic=False, size_pt=11, color=None):
    r = p.add_run(clean_xml_text(txt))
    r.bold       = bold
    r.italic     = italic
...
def _run(para, text, *, bold=False, italic=False, color=None, size_pt=11):
    r = para.add_run(clean_xml_text(text))
    r.bold       = bold
    r.italic     = italic
    r.font.size  = Pt(size_pt)
    r.font.name  = "Arial"
...
def build_word_document(questions, qp_bytes=None, locations=None, progress_cb=None) -> bytes:
    ...
    # Sanitize all text fields in each question before building
    for _q in questions:
        for _fk in ("topic", "ref", "answer", "level", "difficulty",
                    "page_num", "source", "chapter"):
            if _fk in _q and _q[_fk] is not None:
                _q[_fk] = clean_xml_text(_q[_fk])
        # Clean options dict if present
        if "options" in _q and isinstance(_q["options"], dict):
            _q["options"] = {k: clean_xml_text(v) for k, v in _q["options"].items()}
        # Clean lines list if present (MCQ question text lines)
        if "lines" in _q and isinstance(_q["lines"], list):
            _q["lines"] = [clean_xml_text(l) for l in _q["lines"]]
        ...
        for diff_ in sorted_diffs:
            ...
            for q in qs_in_diff:
                ...
                if quote:
                    ...
                    _run(qup, f" : {clean_xml_text(str(quote))}",       size_pt=11)
                else:
                    _hr(doc, color="BFBFBF")
    ...
    try:
        ...
        for _p in doc.paragraphs:
            for _r in _p.runs:
                if _r.text: _r.text = re.sub(r"[\x00-\x08\x0B\x0C\x0E-\x1F\x7F]", "", _r.text)
